...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card/90 backdrop-blur-xl text-foreground p-6 shadow-2xl transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500",
  {
    variants: {
      side: {
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      <SheetPrimitive.Close className="absolute right-5 top-5 rounded-md p-1 text-muted-foreground opacity-70 ring-offset-background transition-all duration-150 ease-in-out hover:opacity-100 hover:bg-primary/10 hover:text-primary focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-secondary">
        <X className="h-5 w-5" />
...
const SheetHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 p-6 text-left border-b border-border/70",
      className
    )}
    {...props}
  />
...
const SheetFooter = ({
  ...
  <div
    className={cn(
      "flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-2 p-6 border-t border-border/70",
      className
    )}
    {...props}
  />
