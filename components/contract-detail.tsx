...
import { Download, Share, AlertTriangle, AlertCircle, CheckCircle, ChevronDown, Send } from "lucide-react"
    ...
    clauses: [
      {
        ...
        risk: "high",
        reason:
          "Die Vertragsstrafe ist überdurchschnittlich hoch und birgt ein erhebliches finanzielles Risiko. Empfehlung: Reduzierung auf 0,1% pro Werktag mit einer Obergrenze von 5%.",
      },
      {
        id: 2,
        section: "§5 Abs. 1",
      ...
      },
      {
        ...
        risk: "low",
        reason: "Diese Klausel entspricht den üblichen Standards und ist akzeptabel.",
      },
      {
        id: 4,
        section: "§9 Abs. 3",
      ...
      <div className="p-6 border-b bg-muted/30">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm">Analyse bearbeiten</Button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-card rounded-lg p-3 border">
            ...
            <div className="text-lg font-semibold">{contract.duration}</div>
          </div>
          <div className="bg-card rounded-lg p-3 border">
            <div className="text-sm text-muted-foreground">Risikobewertung</div>
            <div className="flex items-center gap-2">
              <span className="text-lg font-semibold">{contract.riskScore}%</span>
              <AlertTriangle className="h-4 w-4 text-red-500" />
            </div>
          </div>
          <div className="bg-card rounded-lg p-3 border">
            <div className="text-sm text-muted-foreground">Verhandlungschance</div>
            <div className="text-lg font-semibold">{contract.negotiationProbability}%</div>
          </div>
        </div>
      </div>

      <Tabs defaultValue="klauseln" className="p-6">
            ...
            <div className="divide-y">
              {contract.clauses.map((clause) => (
                <Collapsible
                  ...
                  onOpenChange={() => setOpenClause(openClause === clause.id ? null : clause.id)}
                >
                  ...
                  <CollapsibleContent>
                    ...
                    <div className="space-y-4 p-4 pt-0">
                      ...
                      <div
                        className={`rounded-md border p-3 text-gray-900 dark:text-gray-900 ${getRiskColor(clause.risk)}`}
                      >
                        <div className="mb-1 flex items-center gap-2">
                          <span className="font-medium">{getRiskLabel(clause.risk)}</span>
                        </div>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between text-sm mb-1">
                      ...
                      <span className="font-medium">{contract.riskScore}%</span>
                    </div>
                    <Progress value={contract.riskScore} className="h-2">
                      <div
                        className={`h-full ${getProgressColor(contract.riskScore)} rounded-full`}
              ...
              <CardContent className="pt-6">
                ...
                <div className="space-y-4">
                  <div className="flex flex-col items-center justify-center space-y-2">
                    <div
                      className={`text-4xl font-bold ${contract.negotiationProbability < 50 ? "text-red-500" : contract.negotiationProbability < 75 ? "text-amber-500" : "text-green-500"}`}
                    >
                      {contract.negotiationProbability}%
                    </div>
                    <Progress value={contract.negotiationProbability} className="h-2 w-full">
                      <div
                        className={`h-full ${getNegotiationColor(contract.negotiationProbability)} rounded-full`}
                        style={{ width: `${contract.negotiationProbability}%` }}
                      />
                    </Progress>
        ...
        <div id="negotiation-clauses" className="p-6"></div>
      </Tabs>
    </div>
  )
}
